...
function photoMoment(id,title,narration){state.events.add(id);state.locked=true;safePair(1);const cx=(state.x+state.hx)/2,cy=Math.max(state.y,state.hy);actionAt('photo',cx,cy);const fx=$('#photoFx'),count=$('#photoCountdown');fx.hidden=false;fx.className='photo-fx';count.textContent='3';setObjective('拍照 · 站好不要动');setTimeout(()=>count.textContent='2',600);setTimeout(()=>count.textContent='1',1200);setTimeout(()=>{count.textContent='';fx.classList.add('flash')},1800);setTimeout(()=>{fx.classList.add('saved');localStorage.setItem(`memory-${id}`,'1')},2200);setTimeout(()=>{fx.hidden=true;transition(narration,()=>{endAction();setObjective(`已完成：${title}`);heartBurst(cx,cy)})},3200)}
...
function endAction(){state.locked=false;$('#actionScene').hidden=true;$('#photoFx').hidden=true;$('#player').hidden=false;$('#hedy').hidden=false;placeActors();checkNearby()}
